...
- **jw_cadの実際にグレーアウトしている（無効な）コマンドを、パレット側で判別せずに送信できてしまう問題**（例: Blk化がグレーアウトしていてもパレットからは押せた）。対応案は2つ提示済み、未着手:
  - (a) 送信後にステータスバーの実際の変化を見て、変化がなければ凹みを戻す（軽め）
  - (b) jw_cad実ツールバーのボタン有効/無効状態をTB_GETSTATEで都度確認してから送る（重め、正確）
